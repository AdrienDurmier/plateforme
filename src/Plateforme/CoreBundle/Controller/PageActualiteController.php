...
class PageActualiteController extends Controller
{
    ...
    public function deleteAction($id, Request $request)
    {
      ...
      if ($request->isMethod('POST')) {
        ...
        $em->remove($actualite);
        $em->flush();
        $request->getSession()->getFlashBag()->add('info', "Actualite supprimée avec succès");
        return $this->redirectToRoute('plateforme_core_page_actualites_crud');
      }
      
      return $this->render('PlateformeCoreBundle:PageActualite:delete.html.twig', array(
        'actualite' => $actualite,
        'form' => $form->createView(),
      ));
    }
